...
with st.sidebar:
    ...
    timeout_duration = st.slider("Gemini API Timeout (seconds)", min_value=30, max_value=180, value=90, step=10,
                                 help="Time to wait for a response from Gemini before retrying.")
    retry_attempts = st.slider("Gemini API Retry Attempts", min_value=1, max_value=5, value=4,
                               help="Number of retries if Gemini API fails due to being overloaded.")
...
# --- 3. ENHANCED CORE FUNCTIONS WITH RESILIENT RETRY LOGIC ---

@st.cache_data(show_spinner=False, ttl=3600)  # Cache for 1 hour
def fetch_comments_optimized(video_id: str, api_key: str, max_comments_to_fetch: int = 500):
    """Enhanced comment fetching with better error handling and progress tracking"""
    if not api_key:
        st.error("YouTube API Key is missing. Please enter it in the sidebar.")
        return None
    ...
    try:
        ...
        while len(comments) < max_comments_to_fetch:
            try:
                request = youtube.commentThreads().list(
                    part="snippet",
                    videoId=video_id,
                    textFormat="plainText",
                    maxResults=min(100, max_comments_to_fetch - len(comments)),
                    pageToken=next_page_token,
                    order='time'
                )
                response = request.execute()
                ...
                progress = min(len(comments) / max_comments_to_fetch, 1.0)
                progress_bar.progress(progress)
                progress_container.text(f"📥 Fetched {len(comments)} of {max_comments_to_fetch} comments...")
                ...
                if e.resp.status == 403:
                    st.error("🚫 YouTube API Error: Quota exceeded or API key invalid. Please check your key and try again later.")
                    return None
                elif e.resp.status == 404:
                    st.error(f"❌ YouTube API Error: Video not found or comments are disabled for this video.")
                    return None
                else:
                    st.error(f"❌ An unexpected YouTube API error occurred: {e}")
                    return None
        ...
        st.error(f"❌ Failed to build YouTube service. Check if the API key is valid: {str(e)}")
        return None

@st.cache_data(ttl=3600)
def analyze_sentiments_enhanced(comments: list) -> pd.DataFrame:
    """Enhanced sentiment analysis with additional metrics"""
    if not comments:
        return pd.DataFrame()
    ...
    sentiments = df['text'].apply(lambda text: sid.polarity_scores(str(text)))
...
def call_gemini_api_with_retry(prompt: str, api_key: str, max_retries: int, timeout: int) -> str:
    """
    Calls the Gemini API with a robust retry mechanism, including exponential backoff and jitter.
    This is critical for handling 503 "model overloaded" errors.
    """
    if not api_key:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Specifically handle 503 Service Unavailable (overloaded model)
            if e.response.status_code == 503:
                wait_time = (2 ** attempt) + random.uniform(0, 1) # Exponential backoff with jitter
                logger.warning(f"Gemini API is overloaded (503). Retrying in {wait_time:.2f} seconds... (Attempt {attempt + 1}/{max_retries})")
                time.sleep(wait_time)
            elif e.response.status_code == 429:
                return "Error: Gemini API rate limit exceeded. Please wait and try again later."
            else:
                return f"Error: An HTTP error occurred: {e.response.status_code} - {e.response.text}"
        
        except requests.exceptions.Timeout:
            logger.warning(f"Request timed out. (Attempt {attempt + 1}/{max_retries})")
            if attempt == max_retries - 1:
                return f"Error: Request timed out after {max_retries} attempts. The API is too slow to respond. Try increasing the timeout in Advanced Settings."

        except (KeyError, IndexError, json.JSONDecodeError):
            return "Error: Received an invalid response format from the Gemini API."
            
        except Exception as e:
            logger.error(f"An unexpected error occurred: {e}")
            if attempt == max_retries - 1:
                return f"Error: An unexpected error occurred: {str(e)}"
            time.sleep(2 ** attempt) # Wait before retrying on other unexpected errors

    return f"Error: The model is overloaded. Failed to get a response from Gemini API after {max_retries} attempts. Please try again later."


@st.cache_data(show_spinner=False, ttl=1800)
def get_general_summary_enhanced(comments_tuple: tuple, api_key: str, retries: int, timeout: int) -> str:
    prompt = f"""
    Analyze these YouTube comments and provide a concise, insightful summary.

    1.  **Overall Sentiment**: Describe the general mood (e.g., Overwhelmingly Positive, Mixed but leaning Negative, etc.).
    2.  **Key Topics**: List the 3-5 most discussed themes or topics.
    3.  **Actionable Insights**: Provide 2-3 specific, actionable insights for the video creator. What should they do next based on this feedback?
    
    Comments to analyze (first 300): {comments_tuple}
    """
    return call_gemini_api_with_retry(prompt, api_key, retries, timeout)

@st.cache_data(show_spinner=False, ttl=1800)
def get_custom_summary_enhanced(comments_tuple: tuple, api_key: str, custom_prompt: str, retries: int, timeout: int) -> str:
    prompt = f"""
    Based *only* on the provided YouTube comments, answer the following user question.
    User Question: "{custom_prompt}"

    Analyze the comments thoroughly and provide a direct, evidence-based answer. If the comments do not contain information to answer the question, state that clearly.
...
def display_enhanced_metrics(sentiment_df: pd.DataFrame):
    ...
    with col1:
        ...
        <div class="metric-card">
            <h3>✅ Positive</h3>
            <h2>{positive_count}</h2>
            <p>{(positive_count / total_comments) * 100:.1f}%</p>
    ...
    with col2:
        ...
        <div class="metric-card">
            <h3>❌ Negative</h3>
            <h2>{negative_count}</h2>
            <p>{(negative_count / total_comments) * 100:.1f}%</p>
        </div>""", unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="metric-card">
            <h3>😐 Neutral</h3>
            <h2>{neutral_count}</h2>
            <p>{(neutral_count / total_comments) * 100:.1f}%</p>
        </div>""", unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div class="metric-card">
            <h3>📊 Avg. Score</h3>
            <h2>{avg_sentiment:.3f}</h2>
            <p>{"😊" if avg_sentiment > 0.05 else "😢" if avg_sentiment < -0.05 else "😐"}</p>
        </div>""", unsafe_allow_html=True)

def show_contact_info(query_type: str):
    st.markdown(f"""
    <div class="contact-container">
        <h4 style="color:#FFF;">Contact me for {query_type}</h4>
        <p>I'm happy to help! Please include details about your request for a faster response.</p>
        <a href="https://wa.me/919360734551?text=Hi! I have a {query_type} for your YouTube Analyzer App." target="_blank" class="contact-button">
            📱 WhatsApp Me
        </a>
        <a href="mailto:sanjaydharand@gmail.com?subject=YouTube Analyzer - {query_type}&body=Hi! I have a {query_type} regarding the YouTube Comment Analyzer app." target="_blank" class="contact-button">
...
<div style="text-align: center; margin-bottom: 30px;">
    <h1>⚡ YouTube Comment Analyzer Pro</h1>
    <p style="font-size: 18px; color: #666;">🚀 AI-powered comment analysis to uncover valuable insights</p>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    col1, col2, col3 = st.columns([1, 3, 1])
    with col2:
        video_url_1 = st.text_input("Enter YouTube Video URL", key="url1", placeholder="https://www.youtube.com/watch?v=dQw4w9WgXcQ")
        analyze_button = st.button("🔍 Analyze Video", key="analyze1", use_container_width=True)
    
    if analyze_button and video_url_1:
        if not (video_id := extract_video_id(video_url_1)):
            st.error("❌ Invalid URL. Please enter a valid YouTube video URL.")
            st.stop()
        ...
        if not comments:
            st.warning("Could not fetch comments. The video might have comments disabled, or the API key is invalid.")
            st.stop()
        ...
        if sentiment_df.empty:
            st.info("✅ Comments fetched, but no text was found to analyze.")
            st.stop()
        
        comments_for_ai = tuple(sentiment_df['text'].dropna().head(300).tolist())
        
        spinner_text = "🔄 Step 3/3: Generating AI insights (may take longer due to high demand)..."
        with st.spinner(spinner_text):
            ai_summary = get_general_summary_enhanced(
                comments_for_ai, st.session_state.gemini_api_key, retry_attempts, timeout_duration
            )
        
        st.markdown("<h3>🤖 AI-Generated Insights</h3>", unsafe_allow_html=True)
        if ai_summary.startswith("Error:"):
            st.markdown(f'<div class="error-box"><h4>❌ AI Analysis Failed</h4><p>{ai_summary}</p></div>', unsafe_allow_html=True)
        else:
            st.markdown(f'<div class="success-box"><h4>🧠 AI Analysis Results</h4><p>{ai_summary}</p></div>', unsafe_allow_html=True)
        ...
        with col1:
            ...
            fig_pie = px.pie(
                ...
                color=sentiment_counts.index, color_discrete_map={'Positive':'#4CAF50', 'Negative':'#f44336', 'Neutral':'#9E9E9E'},
        ...
        with col2:
            fig_hist = px.histogram(
                sentiment_df, x='compound', nbins=20, title="Sentiment Score Distribution",
                labels={'compound': 'Sentiment Score'}, color_discrete_sequence=['#667eea']
            )
            st.plotly_chart(fig_hist, use_container_width=True)
        ...
        with col1:
            st.markdown("**Most Positive Comments**")
            for _, row in sentiment_df.nlargest(3, 'compound').iterrows():
                st.markdown(f"""
                <div class="comment-card positive-comment">
                    <p>❤️ {row['text'][:200]}{'...' if len(row['text']) > 200 else ''}</p>
                    <small>Score: {row['compound']:.3f}</small>
                </div>""", unsafe_allow_html=True)
        
        with col2:
            st.markdown("**Most Negative Comments**")
            for _, row in sentiment_df.nsmallest(3, 'compound').iterrows():
                st.markdown(f"""
                <div class="comment-card negative-comment">
...
with tab2:
    ...
    with main_col:
        ...
        if 'prompt_template' in st.session_state:
            custom_prompt = st.text_area("Your specific question about the comments:", value=st.session_state.pop('prompt_template'), key="custom_prompt_area", height=100)
        else:
            custom_prompt = st.text_area("Your specific question about the comments:", placeholder="e.g., What bugs are users reporting?", key="custom_prompt_area", height=100)

        st.markdown("**Quick Templates:**")
        t_col1, t_col2, t_col3 = st.columns(3)
        if t_col1.button("Feature Requests", use_container_width=True): st.session_state.prompt_template = "What features or improvements are users requesting?"; st.rerun()
        if t_col2.button("Bug Reports", use_container_width=True): st.session_state.prompt_template = "What bugs, errors, or issues are users reporting?"; st.rerun()
        if t_col3.button("General Feedback", use_container_width=True): st.session_state.prompt_template = "What is the general feedback and what do users like most about this?"; st.rerun()
        
        analyze_button_2 = st.button("Get Custom Insights", key="analyze2", use_container_width=True)

    if analyze_button_2 and video_url_2 and custom_prompt:
        if video_id := extract_video_id(video_url_2):
            with st.spinner("Fetching and analyzing comments for your query..."):
                comments = fetch_comments_optimized(video_id, st.session_state.youtube_api_key, max_comments)
                if comments:
                    comments_tuple = tuple(pd.DataFrame(comments)['text'].dropna().head(300).tolist())
                    ai_response = get_custom_summary_enhanced(comments_tuple, st.session_state.gemini_api_key, custom_prompt, retry_attempts, timeout_duration)
                    st.subheader("🔮 AI Response to Your Query")
                    st.markdown(f'<div class="{"info-box" if not ai_response.startswith("Error:") else "error-box"}">{ai_response}</div>', unsafe_allow_html=True)
                else:
                    st.warning("Could not fetch comments for this video.")
        else:
            st.error("❌ Invalid URL. Please enter a valid YouTube video URL.")
    elif analyze_button_2:
        st.warning("⚠️ Please provide both a valid video URL and your custom question.")
